...
class ItemModel(BaseModel):
    ...
    measurement_unit: str = Field(
        description="measurement unit if provided. If not provided use \"pcs\"")
    discount: float
    price: float
    vat: float
...
class InvoiceModel(BaseModel):
    ...
    payment_due_date: str = Field(
        description="Payment due date in format yyyy-MM-dd, or empty string if theres no such date in invoice")
    vendor_name: str
    customer_name: str
    currency_code: str
